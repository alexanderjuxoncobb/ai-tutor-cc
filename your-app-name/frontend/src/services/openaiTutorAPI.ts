...
export class OpenAITutorService {
  ...
  async startVoiceConversation(): Promise<void> {
    try {
      console.log('📞 Starting OpenAI voice conversation (simulated real-time)...');
      console.log('⚠️ Note: Browser WebSocket limitation prevents direct Realtime API connection');
      console.log('💡 Using OpenAI Chat API with speech synthesis for voice experience');
  ...
  private async sendInitialGreeting(): Promise<void> {
    let greeting = "Hello! I'm your AI math tutor, and I'm excited to help you with this problem. ";
    
    if (this.context.mathProblem) {
      greeting += "I can see you've uploaded a simultaneous equations problem. This is a great topic to work on together! ";
      greeting += "Would you like to start by telling me what you already know about solving systems of equations, or would you prefer I guide you through the first steps?";
    } else {
      greeting += "I'm ready to help you with any math problem. Feel free to ask me questions or show me your work on the whiteboard!";
    }
  ...
  private buildSystemInstructions(): string {
    let instructions = `You are an expert GCSE mathematics tutor having a voice conversation with a student. 
    ...
    if (this.context.mathProblem) {
      instructions += `\n\nCurrent Math Problem Context:\n${this.context.mathProblem}`;
    }

    // Add recent whiteboard analysis if available
    if (this.context.whiteboardAnalysis && this.context.whiteboardAnalysis.length > 0) {
      const recentAnalysis = this.context.whiteboardAnalysis.slice(-3).join('\n\n');
